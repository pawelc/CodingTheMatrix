...
from vec import Vec
from matutil import listlist2mat,identity
...
# For each part, please provide your solution as a list of the coefficients for
# the generators of V.
#solve(listlist2mat([[0,0,one,one],[one,0,0,one],[0,one,0,one],[one,0,one,one]]),list2vec([one,0,0,0]))
...
#solve(listlist2mat([[one,one,0,0,0,0,0,0],[0,one,one,0,0,0,0,0],[one,0,0,one,0,0,0,0],[0,one,0,0,one,0,0,0],[0,0,one,0,one,0,0,0],[0,0,0,one,one,0,0,0],[0,0,0,0,0,one,0,one],[0,0,0,0,0,0,one,one]]),list2vec([0,one,0,one,0,0,0,0]))
...
lin_dep_R_1 = [2,-1,-1]
lin_dep_R_2 = [-28,7,-4]
lin_dep_R_3 = [155.1750,6.5625,-1, -38.8125,0]
...
#solve(coldict2mat([a0,a1,a2]),identity(set(range(3)), 1))
...
def superset_basis(S, L):
    ...
    for si in range(len(L)):
        if not is_superfluous(T + [L[si]], len(T)):
            T.append(L[si])
